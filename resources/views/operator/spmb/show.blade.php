...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('operator.spmb.index') }}" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition font-semibold">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if($pembayaran->bukti_pembayaran)
                                        <a href="{{ $pembayaran->bukti_pembayaran }}" target="_blank" class="inline-flex items-center px-3 py-1 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if($pembayaran->status === 'pending' && $pembayaran->bukti_pembayaran)
                                        <form action="{{ route('operator.pembayaran.verify', $pembayaran->id) }}" method="POST" class="inline" onsubmit="return confirm('Verifikasi pembayaran ini sebagai LUNAS?')">
                                            @csrf
...
<div id="verifyModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-lg bg-white">
        <div class="mt-3">
            ...
            <form method="POST" action="{{ route('operator.spmb.verify', $pendaftar->id) }}">
                @csrf
                <div class="mb-4">
