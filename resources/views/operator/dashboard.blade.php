...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <x-stat-card
            ...
            :value="number_format($totalPembayaran)"
            color="green"
        ...
        <x-stat-card
            title="Lunas"
            :value="number_format($pembayaranLunas)"
            color="green"
        ...
        <x-stat-card
            title="Terlambat"
            :value="number_format($pembayaranTerlambat)"
            color="red"
    ...
    <x-islamic-card class="border-t-4 border-yellow-600">
        <div class="bg-gradient-to-r from-yellow-100 to-yellow-200 rounded-lg p-8 text-center border-2 border-yellow-400">
            ...
            <p class="text-5xl font-bold text-yellow-800">Rp {{ number_format($totalPendapatan, 0, ',', '.') }}</p>
            <p class="text-sm text-gray-600 mt-2">Tahun Akademik {{ now()->year }}/{{ now()->year + 1 }}</p>
            <div class="mt-6 flex justify-center space-x-8">
                <div>
                    <p class="text-xs text-gray-600">Semester Ganjil</p>
                    <p class="text-xl font-bold text-green-700">Rp {{ $pendapatanGanjil >= 1000000 ? number_format($pendapatanGanjil / 1000000, 1) . ' M' : number_format($pendapatanGanjil, 0, ',', '.') }}</p>
                </div>
                <div class="w-px bg-yellow-400"></div>
                <div>
                    <p class="text-xs text-gray-600">Semester Genap</p>
                    <p class="text-xl font-bold text-green-700">Rp {{ $pendapatanGenap >= 1000000 ? number_format($pendapatanGenap / 1000000, 1) . ' M' : number_format($pendapatanGenap, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
    </x-islamic-card>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <x-islamic-card title="Pembayaran Pending Verifikasi">
            <div class="space-y-3">
                @forelse($pendingPembayaran as $pembayaran)
                <div class="flex items-center justify-between p-4 bg-yellow-50 border border-yellow-200 rounded-lg hover:shadow-md transition-shadow">
                    <div class="flex-1">
                        <div class="flex items-center space-x-2">
                            <span class="font-semibold text-gray-800">{{ $pembayaran->mahasiswa->nama_lengkap }}</span>
                            <span class="text-xs text-gray-500">{{ $pembayaran->mahasiswa->nim }}</span>
                        </div>
                        <p class="text-sm text-gray-600 mt-1">{{ ucwords(str_replace('_', ' ', $pembayaran->jenis_pembayaran)) }} - Rp {{ number_format($pembayaran->jumlah, 0, ',', '.') }}</p>
                        <p class="text-xs text-gray-500 mt-1">{{ $pembayaran->created_at->diffForHumans() }}</p>
                    </div>
                    <div class="flex items-center space-x-2">
                        <x-status-badge status="pending" type="payment" />
                        <a href="{{ route('operator.pembayaran.show', $pembayaran->id) }}" class="px-3 py-1 bg-green-600 text-white text-xs font-semibold rounded-lg hover:bg-green-700 transition-colors">
                            Verifikasi
                        </a>
                    </div>
                </div>
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between p-4 bg-green-50 border border-green-200 rounded-lg hover:shadow-md transition-shadow">
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-600 mt-1">{{ ucwords(str_replace('_', ' ', $pembayaran->jenis_pembayaran)) }} - Rp {{ number_format($pembayaran->jumlah, 0, ',', '.') }}</p>
                        <p class="text-xs text-gray-500 mt-1">{{ $pembayaran->tanggal_bayar ? $pembayaran->tanggal_bayar->format('d/m/Y H:i') : '-' }}</p>
                    </div>
                    <div>
    ...
    <x-islamic-card title="Statistik Pembayaran Bulan Ini">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="text-center p-4 bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg border border-blue-200">
                <p class="text-2xl font-bold text-blue-700">{{ $pembayaranBulanIni['spp'] }}</p>
            ...
            <div class="text-center p-4 bg-gradient-to-br from-green-50 to-green-100 rounded-lg border border-green-200">
                <p class="text-2xl font-bold text-green-700">{{ $pembayaranBulanIni['ukt'] }}</p>
            ...
            <div class="text-center p-4 bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg border border-yellow-200">
                <p class="text-2xl font-bold text-yellow-700">{{ $pembayaranBulanIni['daftar_ulang'] }}</p>
            ...
            <div class="text-center p-4 bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg border border-purple-200">
                <p class="text-2xl font-bold text-purple-700">{{ $pembayaranBulanIni['wisuda'] }}</p>
