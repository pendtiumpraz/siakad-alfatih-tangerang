...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 mt-1">Perbarui pengumuman untuk mahasiswa</p>
        </div>
        <a href="{{ route('admin.pengumuman.index') }}" class="px-6 py-2 bg-gray-500 text-white font-semibold rounded-lg hover:bg-gray-600 transition">
    ...
    <div class="bg-white rounded-lg shadow-md border border-[#D4AF37] overflow-hidden">
        <div class="bg-gradient-to-r from-[#2D5F3F] to-[#4A7C59] px-6 py-4">
            <h2 class="text-xl font-bold text-white">Form Edit Pengumuman</h2>
        </div>

        <form action="{{ route('admin.pengumuman.update', $pengumuman->id) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select id="tipe"
                    ...
                    <option value="penting" {{ old('tipe', $pengumuman->tipe) == 'penting' ? 'selected' : '' }}>Penting</option>
                    <option value="pengingat" {{ old('tipe', $pengumuman->tipe) == 'pengingat' ? 'selected' : '' }}>Pengingat</option>
                    <option value="kegiatan" {{ old('tipe', $pengumuman->tipe) == 'kegiatan' ? 'selected' : '' }}>Kegiatan</option>
                </select>
                @error('tipe')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            ...
            <div class="space-y-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
                <div class="flex items-center">
                    <input type="checkbox"
                           ...
                           value="1"
                           {{ old('untuk_mahasiswa', $pengumuman->untuk_mahasiswa) ? 'checked' : '' }}
