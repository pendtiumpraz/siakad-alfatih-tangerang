...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row gap-4 justify-between">
        <div class="flex gap-2">
            <a href="{{ route('admin.mata-kuliah.create') }}" class="px-6 py-2 bg-gradient-to-r from-[#2D5F3F] to-[#4A7C59] text-white font-semibold rounded-lg hover:shadow-lg transition text-center">
                <i class="fas fa-plus mr-2"></i>
                Tambah Mata Kuliah
            </a>

            @if(request('trashed'))
                <a href="{{ route('admin.mata-kuliah.index') }}" class="px-6 py-2 bg-gray-500 text-white font-semibold rounded-lg hover:shadow-lg transition text-center">
                    <i class="fas fa-arrow-left mr-2"></i>
                    Kembali
                </a>
            @else
                <a href="{{ route('admin.mata-kuliah.index', ['trashed' => 1]) }}" class="px-6 py-2 bg-[#D4AF37] text-white font-semibold rounded-lg hover:shadow-lg transition text-center">
    ...
    @if(!request('trashed'))
        @include('components.batch-delete-actions', ['routeName' => route('admin.mata-kuliah.batch-delete')])
    @else
        @include('components.batch-restore-actions', ['routeName' => route('admin.mata-kuliah.batch-restore')])
    @endif
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($mataKuliahs ?? [] as $index => $mk)
                        <tr class="{{ $mk->trashed() ? 'bg-red-50 hover:bg-red-100' : 'hover:bg-[#F4E5C3] hover:bg-opacity-30' }} transition">
                            @if(!request('trashed'))
                            <td class="px-4 py-4">
                                <input type="checkbox" class="row-checkbox w-4 h-4 text-green-600 border-gray-300 rounded focus:ring-green-500" value="{{ $mk->id }}" onchange="updateSelectedIds()">
                            </td>
                            @else
                            <td class="px-4 py-4">
                                <input type="checkbox" class="restore-row-checkbox w-4 h-4 text-green-600 border-gray-300 rounded focus:ring-green-500" value="{{ $mk->id }}" onchange="updateRestoreSelectedIds()">
                            </td>
                            @endif
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    @if(request('trashed'))
                                        <form action="{{ route('admin.mata-kuliah.restore', $mk->id) }}" method="POST" class="inline">
                                            @csrf
                                    ...
                                    @else
                                        <a href="{{ route('admin.mata-kuliah.show', $mk->id) }}" class="text-blue-600 hover:text-blue-800 transition" title="Detail">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('admin.mata-kuliah.edit', $mk->id) }}" class="text-[#D4AF37] hover:text-[#b8941f] transition" title="Edit">
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ request('trashed') ? 8 : 9 }}" class="px-6 py-8 text-center text-gray-500">
