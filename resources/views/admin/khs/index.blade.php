...
<div class="space-y-6">
    <!-- Page Header -->
    <x-admin.page-header title="Daftar KHS (Kartu Hasil Studi)" />
    ...
    <div class="bg-white rounded-lg shadow-md border-2 border-[#D4AF37] p-6">
        ...
        <form action="{{ route('admin.khs.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <!-- Semester Filter -->
            <div>
            ...
            <div>
                ...
                <select name="program_studi_id" class="w-full px-4 py-2 border-2 border-[#2D5F3F] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#D4AF37]">
            ...
            <!-- IP Min Filter -->
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">IP Minimal</label>
                <input type="number" name="ip_min" step="0.01" min="0" max="4" value="{{ request('ip_min') }}" class="w-full px-4 py-2 border-2 border-[#2D5F3F] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#D4AF37]" placeholder="0.00">
            </div>

            <!-- IP Max Filter -->
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">IP Maksimal</label>
                <input type="number" name="ip_max" step="0.01" min="0" max="4" value="{{ request('ip_max') }}" class="w-full px-4 py-2 border-2 border-[#2D5F3F] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#D4AF37]" placeholder="4.00">
            </div>

            <!-- Search Filter -->
            <div class="md:col-span-2">
                <label class="block text-sm font-semibold text-gray-700 mb-2">Cari NIM/Nama</label>
                <input type="text" name="search" value="{{ request('search') }}" class="w-full px-4 py-2 border-2 border-[#2D5F3F] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#D4AF37]" placeholder="Masukkan NIM atau Nama Mahasiswa">
            </div>
            ...
            <div class="md:col-span-2 flex items-end gap-2">
                ...
                <a href="{{ route('admin.khs.index') }}" class="px-6 py-2 bg-gray-500 text-white font-semibold rounded-lg hover:bg-gray-600 transition">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div class="bg-white rounded-lg shadow-md border-l-4 border-yellow-500 p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-500 font-semibold">IP Tertinggi</p>
                    <p class="text-3xl font-bold text-yellow-600">
                        {{ $khsList->count() > 0 ? number_format($khsList->max('ip'), 2) : '0.00' }}
                    </p>
                </div>
        ...
        <div class="overflow-x-auto">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($khsList as $index => $khs)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <a href="{{ route('admin.khs.show', $khs->id) }}" class="inline-flex items-center px-3 py-1 bg-blue-600 text-white text-xs font-semibold rounded hover:bg-blue-700 transition">
            ...
            @else
                <div class="px-6 py-12 text-center">
                    ...
                    <p class="text-gray-400 text-sm mt-2">Silakan generate KHS terlebih dahulu dari halaman semester</p>
                </div>
            @endif
        </div>
    </div>
