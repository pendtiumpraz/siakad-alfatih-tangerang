                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    ...
                                    <form action="{{ route('admin.semester.destroy', $semester->id) }}" method="POST" class="inline"
                                        x-data
                                        @submit.prevent="window.swalConfirmDelete($event, 'semester ini')">
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="bg-gray-50 px-6 py-4 border-t border-gray-200">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex space-x-1">
                        ...
                        @php
                            $current = $semesters->currentPage();
                            $last = $semesters->lastPage();
                            $delta = 2;
                            $left = $current - $delta;
                        ...
                        @foreach ($rangeWithDots as $page)
                            ...
                            @else
                                <a href="{{ $semesters->url($page) }}" class="px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded hover:bg-[#D4AF37] hover:text-white transition">{{ $page }}</a>
                            @endif
                        @endforeach
