...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex space-x-3">
            <a href="{{ route('admin.spmb.export', request()->query()) }}" class="px-4 py-2 bg-gradient-to-r from-[#D4AF37] to-[#F4E5B0] text-gray-800 rounded-lg hover:shadow-lg transition-all font-semibold flex items-center space-x-2">
    ...
    <div class="bg-white rounded-lg shadow-md border border-[#D4AF37] p-6">
        ...
        <form method="GET" action="{{ route('admin.spmb.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
            ...
            <div class="md:col-span-2 lg:col-span-4 flex justify-end space-x-2">
                ...
                <a href="{{ route('admin.spmb.index') }}" class="px-6 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition-colors font-semibold">
                    Reset
                </a>
            </div>
        </form>
                    ...
                    @forelse($pendaftars as $pendaftar)
                        <tr class="hover:bg-green-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <input type="checkbox" name="pendaftar_ids[]" value="{{ $pendaftar->id }}" class="pendaftar-checkbox rounded border-gray-300">
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <a href="{{ route('admin.spmb.show', $pendaftar->id) }}" class="inline-flex items-center px-3 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700 transition">
        ...
        <div id="bulkActions" class="hidden px-6 py-4 bg-gray-50 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <span class="text-sm text-gray-700">
                    <span id="selectedCount">0</span> pendaftar dipilih
                </span>
                <div class="flex space-x-2">
                    <button type="button" onclick="bulkVerify()" class="px-4 py-2 bg-blue-600 text-white text-sm rounded hover:bg-blue-700 transition">
                        Verifikasi Terpilih
                    </button>
                    <button type="button" onclick="bulkReject()" class="px-4 py-2 bg-red-600 text-white text-sm rounded hover:bg-red-700 transition">
                        Tolak Terpilih
                    </button>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateBulkActions() {
            const checked = document.querySelectorAll('.pendaftar-checkbox:checked');
    ...
    function bulkVerify() {
        ...
        form.method = 'POST';
        form.action = '{{ route("admin.spmb.index") }}/bulk-verify';
    ...
    function bulkReject() {
        ...
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = '{{ route("admin.spmb.index") }}/bulk-reject';
