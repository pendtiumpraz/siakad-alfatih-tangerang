...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="hidden md:block text-right">
            <p class="text-sm text-gray-600">{{ \Carbon\Carbon::now()->locale('id')->isoFormat('dddd, D MMMM YYYY') }}</p>
            <p class="text-xs text-gray-500">{{ $semesterAktif ? $semesterAktif->nama_semester . ' ' . $semesterAktif->tahun_akademik : 'Belum ada semester aktif' }}</p>
        </div>
    </div>
    ...
    <div class="card-islamic p-6 islamic-pattern">
        <div class="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6">
            ...
            <div class="relative">
                ...
                <div class="absolute -bottom-2 -right-2 {{ $stats['status'] == 'aktif' ? 'bg-[#D4AF37]' : 'bg-gray-500' }} text-white text-xs px-2 py-1 rounded-full font-semibold">
            ...
            <div class="flex-1 text-center md:text-left">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mt-4">
                    ...
                    <div class="bg-white/80 px-4 py-2 rounded-lg">
                        ...
                        <p class="font-semibold text-gray-800">Semester {{ $stats['semester_aktif'] }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card-islamic p-6 gold-accent text-white transform hover:scale-105 transition">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-4xl font-bold">{{ number_format($stats['ip_semester'], 2) }}</p>
                </div>
            ...
            <div class="mt-3 text-xs opacity-90">
                <span>{{ $khsTerakhir ? $khsTerakhir->semester->nama_semester . ' ' . $khsTerakhir->semester->tahun_akademik : 'Belum ada data' }}</span>
            </div>
        </div>
        ...
        <div class="card-islamic p-6 green-accent text-white transform hover:scale-105 transition">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-4xl font-bold">{{ number_format($stats['ipk'], 2) }}</p>
                </div>
                <svg class="w-12 h-12 opacity-50" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div class="card-islamic p-6 hover:shadow-lg transition">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-4xl font-bold text-gray-800">{{ $stats['total_sks'] }}</p>
                </div>
            ...
            <div class="mt-3 text-xs text-gray-500">
                <span>SKS yang telah lulus</span>
            </div>
        </div>
        ...
        <div class="card-islamic p-6 hover:shadow-lg transition">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <span class="inline-block px-4 py-2 {{ $stats['status'] == 'aktif' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }} rounded-full font-bold text-lg">
                        {{ ucfirst($stats['status']) }}
                    </span>
                </div>
            ...
            <div class="mt-3 text-xs text-gray-500">
                <span>Terdaftar Semester {{ $stats['semester_aktif'] }}</span>
            </div>
        </div>
    </div>
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-start space-x-4 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
                    <div class="bg-[#4A7C59] text-white px-3 py-2 rounded-lg text-center min-w-[70px]">
                        ...
                        <p class="font-bold">{{ \Carbon\Carbon::parse($jadwal->jam_mulai)->format('H:i') }}</p>
                    </div>
                    <div class="flex-1">
                ...
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-600">Tidak ada jadwal kuliah hari ini</p>
                </div>
                @endforelse
            </div>
        </div>
            ...
            <div class="space-y-3">
                ...
                @forelse($pengumuman as $index => $item)
                    @php
                        ...
                        $icon = $icons[$index % 4];
                    @endphp
                    <div class="flex items-start space-x-3 p-3 bg-{{ $color }}-50 border-l-4 border-{{ $color }}-500 rounded-r-lg hover:bg-{{ $color }}-100 transition cursor-pointer">
                        <svg class="w-5 h-5 text-{{ $color }}-500 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                            {!! $icon !!}
                        </svg>
                        <div class="flex-1">
                            <h4 class="font-semibold text-gray-800 text-sm">{{ $item->judul }}</h4>
                            <p class="text-xs text-gray-600 mt-1">{{ Str::limit($item->isi, 60) }}</p>
        ...
        <div class="flex items-start justify-between">
            <div class="flex items-start space-x-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-sm text-gray-600 mb-3">Anda memiliki {{ $pembayaranPending->count() }} tagihan yang perlu diselesaikan:</p>
                    <div class="space-y-2">
                        @foreach($pembayaranPending as $bayar)
                        <div class="flex items-center space-x-3 bg-white p-3 rounded-lg">
                            <span class="inline-block w-2 h-2 {{ $bayar->tanggal_jatuh_tempo->isPast() ? 'bg-red-500' : 'bg-yellow-500' }} rounded-full"></span>
                            <span class="font-semibold text-gray-800">{{ $bayar->jenis_pembayaran }}</span>
                            <span class="text-sm text-gray-600">- Rp {{ number_format($bayar->jumlah, 0, ',', '.') }}</span>
                            <span class="text-xs {{ $bayar->tanggal_jatuh_tempo->isPast() ? 'text-red-600 bg-red-100' : 'text-yellow-600 bg-yellow-100' }} font-semibold px-2 py-1 rounded">
                                Jatuh tempo: {{ $bayar->tanggal_jatuh_tempo->format('d M Y') }}
                            </span>
                        </div>
                        @endforeach
                    </div>
            ...
            <a href="{{ route('mahasiswa.pembayaran.index') }}"
               class="bg-[#4A7C59] hover:bg-[#3d6849] text-white px-6 py-3 rounded-lg font-semibold transition transform hover:scale-105 self-start">
                Bayar Sekarang
            </a>
        </div>
    </div>
