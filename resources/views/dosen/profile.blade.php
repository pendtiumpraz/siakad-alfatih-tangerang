...
<!-- Modal Edit Profile -->
<div id="editProfileModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 p-4 overflow-y-auto">
    <div class="bg-white rounded-xl shadow-2xl max-w-5xl w-full my-4 flex flex-col max-h-[calc(100vh-2rem)]">
        <!-- Header (Fixed) -->
        <div class="flex-shrink-0 bg-white border-b px-5 py-3 flex items-center justify-between rounded-t-xl">
        ...
        <!-- Scrollable Content -->
        <form id="editProfileForm" action="{{ route('dosen.profile.update') }}" method="POST" enctype="multipart/form-data" class="flex-1 overflow-y-auto p-5 space-y-4">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    @if($dosen->foto)
                        <div class="mb-2">
                            <img src="{{ route('image.proxy', ['id' => $dosen->foto]) }}"
                                 alt="Foto saat ini"
                                 class="w-full h-28 object-cover rounded-lg border-2 border-green-500"
                                 onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($dosen->nama_lengkap) }}&size=200&background=059669&color=fff';">
                    ...
                    <label class="block text-sm font-medium text-gray-600 mb-1">Upload Foto Baru</label>
                    ...
                    <div id="newFotoPreview" class="mt-2 hidden">
                        ...
                        <img id="newFotoImage" src="" alt="Preview" class="w-full h-24 object-cover rounded-lg border-2 border-blue-500">
                    </div>
                </div>
        ...
        <!-- Footer (Fixed) -->
        <div class="flex-shrink-0 bg-white border-t px-5 py-3 flex items-center justify-end space-x-3 rounded-b-xl">
            <button type="button" onclick="closeModal('editProfileModal')" class="px-5 py-2 text-sm bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition font-semibold">
                Batal
            </button>
            <button type="submit" form="editProfileForm" class="px-5 py-2 text-sm bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-semibold">
                Simpan Perubahan
            </button>
        </div>
    </div>
