...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-800">Ajukan Pencairan Gaji</h1>
            <p class="text-gray-600 mt-1">Lengkapi form untuk mengajukan pencairan gaji</p>
        </div>
    ...
    <!-- Form -->
    <form action="{{ route('dosen.penggajian.store') }}" method="POST" class="bg-white rounded-lg shadow-md border border-[#D4AF37] p-6 space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="month" id="periode" name="periode" value="{{ old('periode') }}" 
                ...
                <select id="semester_id" name="semester_id" 
                    ...
                    @foreach($semesters as $semester)
                        <option value="{{ $semester->id }}" {{ old('semester_id') == $semester->id ? 'selected' : '' }}>
                            {{ $semester->jenis }} {{ $semester->tahun_ajaran }}
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="border-t border-gray-200 pt-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="url" id="link_rps" name="link_rps" value="{{ old('link_rps') }}" 
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2D5F3F] focus:border-transparent" 
                ...
                <div>
                    ...
                    <input type="url" id="link_materi_ajar" name="link_materi_ajar" value="{{ old('link_materi_ajar') }}" 
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2D5F3F] focus:border-transparent" 
                        placeholder="https://drive.google.com/..." required>
                </div>
                ...
                <div>
                    ...
                    <input type="url" id="link_absensi" name="link_absensi" value="{{ old('link_absensi') }}" 
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2D5F3F] focus:border-transparent" 
                        placeholder="https://drive.google.com/..." required>
                </div>
            </div>
        ...
        <div class="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200">
            <a href="{{ route('dosen.penggajian.index') }}" class="px-6 py-2 bg-gray-300 text-gray-700 font-semibold rounded-lg hover:bg-gray-400 transition">
                Batalkan
            </a>
